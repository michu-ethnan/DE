...
if (ALERT_BOX.resolveFor(actor).isPresent()){
    ...
    actor.attemptsTo(
            Check.whether(ALERT_BOX.resolveFor(actor).containsText("Das Produkt wurde zum Einkaufswagen hinzugefügt")).andIfSo(MoveMouse.to(ALERT_BOX)).otherwise(ReturnAndAddAnotherProduct.toCart())
    );
}
else {
    actor.attemptsTo(
